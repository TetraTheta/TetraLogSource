...
export default class Renderer {
  ...
  title(result) {
    return this.highlight(
      result.item.title,
      result.matches.filter((match) => match.key === 'title'),
    );
  }
  ...
  desc(result) {
    ...
    return (
      ...
      this.highlight(
        result.item.summary,
        result.matches.filter((match) => match.key === 'summary'),
      ) +
      '</div>'
    );
  }
  ...
  content(result) {
    ...
    if (first > 0) {
      ...
      content =
        (first > this.contentContextLength ? '...' : '') +
        content.substring(Math.max(0, first - this.contentContextLength));
    }
  ...
  <div class="search-result-content">
    ...
    this.getContainer()
      .querySelectorAll('.search-history')
      .forEach((ele) => {
