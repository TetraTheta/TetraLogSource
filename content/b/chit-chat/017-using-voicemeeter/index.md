...
{{< gallery/image src="vm-banana" width="500px" >}}
...
기본적으로 VoiceMeeter는 Windows의 볼륨 조절을 인식하지 않는다.
...
VoiceMeeter의 Menu를 누르면 나오는 메뉴의 Shortcut Key (Hook)에서 다음 항목 중 하나를 체크한다.
...
상단 메뉴의 Options를 누르면 나오는 Enable Windows Volume Control을 체크한다.

이 방법은 Windows 볼륨 슬라이더가 나타나긴 하지만, 음소거 버튼을 눌렀을 때 볼륨이 즉각적으로 꺼지는 게 아니라 페이드 아웃으로 점진적으로 줄어든다. 음소거를 풀 때에도 역시 페이드 인으로 볼륨이 증가하는 방식이다.
...
Windows API와 VoiceMeeter API를 둘 다 사용하기 때문에, 앞선 방식과 다르게 Windows 볼륨 슬라이더와 즉각적 볼륨 반영을 경험할 수 있다.
...
Voicemeeter Windows Volume보다 기능은 약간 떨어지지만, Windows 볼륨 슬라이더와 VoiceMeeter를 연결한다는 가장 핵심적인 기능은 여전히 유지하고 있다.

실행 파일의 크기가 1MB 미만으로 매우 가볍기 때문에 현재 사용 중인 프로그램이다.
