...
> 3. 해당 스크립트는 실행 인수를 지원하지 않습니다. 실행 인수가 필요하다면 스크립트 파일을 직접 수정하세요.
...
Windows 11의 보안 강화로 인해 `net`, `mshta` 명령어가 스크립트 내에서 정상 작동하지 않습니다. 따라서 해당 명령어를 [`gsudo`](https://github.com/gerardog/gsudo)와 `powershell`로 대체하였습니다.
...
0. 스크립트 실행 시, 첫 번째 실행 인수가 `TASKRUN`인지 확인합니다.
